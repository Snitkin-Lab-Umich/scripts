...
sub convertAlnToFile{
  ...
  if($$settings{concatenateAlignment}){
	...
	my $temp_seq = $seq->seq;
	$temp_seq =~ s/[^ACTGactg-]/N/g;
	$temp_seq =~ tr/a-z/A-Z/;
        $alnSequence{$seq->id}.=$temp_seq.$$settings{linker};
      }
	...
	print "$id\n";
      ...
      my $seq=new Bio::LocatableSeq(-seq=>$alnSequence{$_},-id=>$id); # must be locatableSeq to be added
      $concatAln->add_seq($seq);
    }
	$concatAln->set_displayname_flat();
